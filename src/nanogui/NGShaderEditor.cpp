...
NGShaderEditor::NGShaderEditor(Screen *parent) : nanogui::Window(parent, "Shader Parameters"),
mPointRadius(0.1)
{
    this->setPosition(Vector2i(0, 0));

    GridLayout * group = new GridLayout(Orientation::Vertical);
    group->setColAlignment({ Alignment::Maximum, Alignment::Fill });
    group->setSpacing(0, 10);
    ...
    this->addParameter(panel, "Point Radius:", "0.1");
    this->addParameter(panel, "Trim Radius:", "1000000.0");
    this->addParameter(panel, "Exposure:", "1.0");
    this->addParameter(panel, "Contrast:", "1.0");
    ...
    this->addDropDown(panel, "Colour Mode:", { "Intensity", "Colour", "Return Index", "Point Source", "Las Classification", "File Number"});
    this->addDropDown(panel, "Selection:", { "All", "Classified", "First Return", "Last Return", "First Of Several"});
}

NGShaderEditor::~NGShaderEditor()
{
}

void NGShaderEditor::addParameter(Widget * panel, const std::string & label, const std::string & value)
{
    ...
    textBox->setFixedSize(Vector2i(80, 24));
    textBox->setValue(value);
    // textbox->setunits("gib");
    textBox->setDefaultValue(value);
    textBox->setFontSize(16);
    textBox->setFormat("[-]?[0-9]*\\.?[0-9]+");
}

void NGShaderEditor::addDropDown(Widget * panel, const std::string & label, const std::vector<std::string> & values)
{
    new Label(panel, label, "sans-bold");

    ComboBox * comboBox = new ComboBox(panel, values);
    comboBox->setFixedSize(Vector2i(140, 24));
    comboBox->setFontSize(20);
}
...
// FIXME: these icons are loaded at runtime ... from a local folder (relative to the CWD)
